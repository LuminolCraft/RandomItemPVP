...
public class GameInstance {
    ...
    public void forceStop() {
        ...
        if (pluginInstance != null) {
            AllyMobManager allyMobManager = pluginInstance.getAllyMobManager();
            if (allyMobManager != null) {
                for (Player player : new ArrayList<>(participants)) {
                    if (player.isOnline()) {
                        allyMobManager.clearPlayerAllies(player.getUniqueId());
                    }
                }
            }
        }
            ...
            Bukkit.getRegionScheduler().run(plugin, pillarBaseLoc, task -> {
                ...
                player.getScheduler().run(plugin, playerTask -> {
                    player.teleportAsync(playerSpawn).thenRun(() -> {
                        ...
                        player.getScheduler().run(plugin, effectTask -> {
    ...
    public void onBlockPlace(org.bukkit.event.block.BlockPlaceEvent event) {
        ...
        if (!alivePlayers.contains(player)) {
            ...
            player.sendActionBar(net.kyori.adventure.text.Component.text("§c你已死亡，无法放置方块！"));
            return;
        }
        
        // 存活参与者可以正常放置方块（不需要记录，因为世界实例会被删除）
    }
    ...
    public void onBlockBreak(org.bukkit.event.block.BlockBreakEvent event) {
        ...
        if (!alivePlayers.contains(player)) {
            event.setCancelled(true);
            player.sendActionBar(net.kyori.adventure.text.Component.text("§c你已死亡，无法破坏方块！"));
        }
    ...
    public void onBucketEmpty(org.bukkit.event.player.PlayerBucketEmptyEvent event) {
        ...
        if (!alivePlayers.contains(player)) {
            event.setCancelled(true);
            player.sendActionBar(net.kyori.adventure.text.Component.text("§c你已死亡，无法使用桶！"));
        }
    ...
    private void stopGame() {
        ...
        if (airdropManager != null) {
            airdropManager.stopAirdropForArena(arena.getArenaName());
        }
