...
[![Version](https://img.shields.io/badge/Version-4.0.0-red.svg)](https://github.com/LuminolCraft/RandomItemPVP/releases)
...
<p align="center">
  <img src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Minecraft%20RandomItemPVP%20plugin%20banner%20with%20players%20fighting%20in%20a%20shrinking%20arena%20with%20random%20items%20and%20special%20abilities&image_size=landscape_16_9" alt="RandomItemPVP Banner" width="800"/>
</p>

<p align="center">
  <strong>⚡ 高性能 | 🎲 强随机性 | 🛠️ 可配置 | 👥 多房间 | 📊 数据统计</strong>
</p>
...
RandomItemPVP 的设计遵循以下几个核心原则：

1. **公平性**：通过随机物品发放机制确保每位玩家在游戏开始时拥有相同的机会，运气也是实力的一部分。

2. **竞技性**：快节奏的游戏设计（每局3-5分钟）要求玩家具备快速反应和灵活战术的能力。

3. **可扩展性**：高度可配置的参数允许服务器管理员根据自身需求调整游戏平衡性，创造独特的游戏体验。

4. **高性能**：充分利用 Folia 的多线程能力，确保在高负载情况下依然保持流畅的游戏体验。

5. **可维护性**：模块化的代码结构和详细的配置选项，便于服务器管理员进行定制和维护。
...
| **特殊物品能力** | TNT 投掷、火焰弹投掷、末影水晶等特殊能力 |
| **空投系统** | 定期从空中降落补给箱，提供高级资源 |
| **击杀奖励系统** | 成功击杀敌人获得生命值恢复和额外物品 |
...
插件引入了物品权重系统，管理员可以为不同物品配置出现概率，稀有物品自然更难获得。药水性状也经过精心设计，所有发放的药水都会自动附带随机有用的效果（如治疗、力量、速度等），避免了获得无用药水的挫败感。

### 🌐 大逃杀机制

经典的缩圈机制迫使所有玩家逐渐靠近，最终在有限的区域内展开决战。边界可以穿越但会持续造成伤害，玩家必须在安全区域内寻找资源和战斗机会。每位玩家在游戏开始时会被传送到高空的基岩柱顶部，围成一个圆圈均匀分布，自动获得缓降效果防止摔落伤害。
...
游戏过程中会随机触发各类事件，为对局增添变数和紧张感。目前支持的事件类型包括：

- **箭雨**：从天而降的箭矢攻击随机玩家
- **僵尸围城**：大量僵尸突然出现在目标玩家周围
- **恶魂袭击**：恶魂向玩家发射火球
- **苦力怕雨**：五只苦力怕从高空降落，自带缓降效果不会摔死

所有事件的目标都是随机存活的玩家，确保公平性。
...
- **TNT**：右键投掷，以抛物线飞向目标位置，2.5秒后爆炸，威力足以破坏地形但不会伤害投掷者。

- **火焰弹**：右键投掷，冷却时间更短（2秒），只点燃目标不造成爆炸伤害，适合快速骚扰和区域封锁。

- **末影水晶**：可以放置在任何固体方块上，被攻击后产生4.0威力的爆炸，是制造大规模破坏和区域控制的利器。

### 📊 数据统计系统

插件集成了完整的数据统计系统。系统记录每位玩家的：
...
统计数据持久化存储，即使服务器重启也不会丢失。排行榜系统支持按胜利次数、击杀数和KD比率三种方式排序，管理员可以通过指令快速查看TOP10玩家。

### 👥 多房间系统

插件引入了强大的多房间支持。每个房间完全独立运行，拥有自己的配置、地图、游戏状态和世界实例，互不干扰。这使得服务器可以同时接待不同水平的玩家群体，或举办不同规则的比赛。

多房间系统的核心是 Worlds 插件的世界实例化功能。每个游戏房间在开始时都会创建一个独立的世界副本，确保游戏的地图状态在每局之间完美重置，不会因为玩家的破坏而影响下一局游戏。

### 🐺 盟友生物系统

游戏中，玩家可以通过空投获得盟友生物的刷怪蛋。使用刷怪蛋可以召唤各种生物作为盟友，协助玩家战斗。盟友生物会自动攻击敌对玩家，并且拥有增强的生命值和攻击力。

管理员可以在配置文件中调整盟友生物的类型、生成概率和行为参数，创造独特的游戏体验。

### 🔧 地图重置系统

插件集成了智能地图重置系统，确保每局游戏的地图状态都能完美恢复。系统会在游戏开始前保存地图的初始状态，游戏结束后自动将地图恢复到初始状态，包括：
...
地图重置系统支持异步操作，不会影响服务器的正常运行，确保在大型服务器上也能流畅运行。
...
│   ├── ConfigBootstrapper.java      # 配置引导器
│   ├── DebugLogger.java             # 调试日志管理器
...
1. 访问项目的 [Releases 页面](https://github.com/LuminolCraft/RandomItemPVP/releases)

2. 下载最新版本的 JAR 文件（RandomItemPVP-4.0.0.jar）
...
```bash
# 克隆仓库
git clone https://github.com/LuminolCraft/RandomItemPVP.git
cd RandomItemPVP
...
# 构建产物位于 target/ 目录（Maven）或 build/libs/ 目录（Gradle）
...
debug:
  enabled: false                  # 主调试开关
  level: basic                    # 调试级别（basic/verbose）
  filter-repeating: true          # 是否过滤重复消息
```
...
- **arenas.yml**：固定房间配置、默认地图设置
- **map-reset.yml**：地图重置参数、保护机制设置
...
- **TNT**：右键投掷，物品会以抛物线飞向准心指向的位置，2.5秒后爆炸并造成区域破坏。

- **火焰弹**：右键投掷，冷却时间更短（2秒），只产生点燃效果不造成爆炸伤害。

- **末影水晶**：可以放置在任何固体方块上，被其他玩家攻击后会产生强力爆炸，是区域控制和战术干扰的利器。
...
插件引入了完整的多房间支持，允许服务器同时运行多个独立的游戏房间。每个房间拥有自己的配置、地图、游戏状态和世界实例，互不干扰。这使得服务器可以同时接待不同水平的玩家群体，或举办不同规则的比赛。
...
每个房间的详细配置存储在 `config-modules/arenas.yml` 文件中。房间配置文件采用以下结构：

```yaml
# 固定房间配置
room1:
  enabled: true
  map: "map1"  # 地图 ID（来自 maps.yml）
...
插件集成了强大的调试功能，允许服务器管理员控制调试信息的输出。调试功能可以帮助管理员排查问题，同时避免调试信息刷屏影响服务器性能。

### 配置调试功能

在 `config.yml` 文件中，找到 `debug` 配置部分：
...
debug:
  ...
  # 是否过滤重复消息（默认为 true）
  # true - 过滤重复的调试消息
  # false - 显示所有调试消息（包括重复的）
  filter-repeating: true
```
...
- **basic**：只显示基本的调试信息，如游戏状态变化、房间创建/删除等重要事件。

- **verbose**：显示详细的调试信息，包括物品发放、事件触发、边界变化等更细致的信息。

### 重复消息过滤

调试功能会自动过滤重复的调试消息，避免刷屏。系统会记录最近5秒内的调试消息，当检测到重复消息时会自动忽略，只显示首次出现的消息。

### 使用方法

1. 打开 `config.yml` 文件

2. 将 `debug.enabled` 设置为 `true`

3. 根据需要调整 `debug.level` 和 `debug.filter-repeating`

4. 使用 `/ripvp reload` 命令热加载配置
...
- **Apache Maven 3.8+** 或 **Gradle 7.0+**

可以通过以下命令检查环境：

```bash
java -version
mvn -version  # 或 gradle -version
```
...
```
target/
├── RandomItemPVP-4.0.0.jar        # 带依赖的完整 JAR
└── original-RandomItemPVP-4.0.0.jar  # 不带依赖的 JAR
```
...
如需修改版本号，可以编辑 `pom.xml` 文件（Maven）或 `build.gradle.kts` 文件（Gradle）：
...
| 渠道 | 链接 |
|------|------|
| **GitHub 主页** | https://github.com/LuminolCraft/RandomItemPVP |
| **Issues 反馈** | https://github.com/LuminolCraft/RandomItemPVP/issues |
| **Releases 下载** | https://github.com/LuminolCraft/RandomItemPVP/releases |
